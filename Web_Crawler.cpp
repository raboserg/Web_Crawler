...
Web_Crawler::~Web_Crawler(void) { delete this->url_visitor_factory_; }

Web_Crawler::Web_Crawler(void) : url_visitor_factory_(0) {}

int Web_Crawler::open(int argc, ACE_TCHAR *argv[]) {
  ...
  // @@ Put the ACE_Service_Config::open() stuff here somewhere...
    ...
    ACE_NEW_RETURN(this->url_visitor_factory_, URL_Validation_Visitor_Factory,
                   -1);
    return 0;
  }
}

int Web_Crawler::run(void) {
  // Make the appropriate <URL_Visitor>.
  std::shared_ptr<URL_Visitor> visitor(
      this->url_visitor_factory_->make_visitor());
  ...
  std::shared_ptr<Command_Processor> cp(
      this->url_visitor_factory_->make_command_processor());

	if (cp == 0)
    ACE_ERROR_RETURN((LM_ERROR, "%p\n", "make_command_processor"), -1);
  ...
  ACE_DEBUG(
      (LM_DEBUG, ACE_TEXT("URl Address !\n"), url_addr->addr_to_string()));

  HTTP_URL *http_url;
  ACE_NEW_RETURN(http_url, HTTP_URL(**url_addr_ptr), 0);

  Auto_Destroyer<HTTP_URL> http_url_ptr(http_url);

  URL_Command *url_command;
  ACE_NEW_RETURN(url_command, URL_Command(*http_url_ptr), 0);
